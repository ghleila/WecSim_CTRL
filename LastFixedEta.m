...
opts.MaxIter = 100;
% opts.BatchSize = 50;   %% could be num/4;
opts.Verbose = false;
opts.StepRatio = 0.01;
...
nodes = [size(Input_init,2) 16 10 size(action,1)];
...
%     init_stIdx=ini_epis_state_idx(1,episode);

   % state_idx=ini_epis_state_idx(1,1);  %%% initial state to start
    state_idx=2*121+1;
    ...
    ove_count=0;
     for jk=1:1500  %%while dif>0.00001 %  
...
%         end
    ...
    eta=SSnETA(eta_indx,3:end)';
        ...
        [All_params3,next_state_idx, PptoAvg]=SIMULATOR2(FeWithTime,A1,B1,K1,B2,K2,tsim,StepT_SIM,state_idx);   % initiaL STATE
        next_reward=(PptoAvg/1e03); %% normalized reward
