...
CFG.lambda = 0;                     % eligibility trace parameter (0 to disable elig tr)
...
if cfg.run,
    ...
    % -----------------------------------------------
    % Perform Q-iteration
    if cfg.lambda > 0, dispx('Performing SARSA(lambda)...', cfg.verb, 0);
    else dispx('Performing SARSA...', cfg.verb, 0);
    end;
    ...
    while trial <= cfg.maxtrials && ~conv,       % main loop
        ...
        while k < cfg.maxsteps && ~terminal,        
            ...
            % choose action in the NEXT state (required to compute the
            % temporal difference)
            if rand < explor,   % exploratory
                uindplus = unidrnd(DIMS.M);
            else                % greedy action
                [Qmax uindplus] = find(Q(xindplus, :) == max(Q(xindplus, :)));
                if length(uindplus) > 1, uindplus = uindplus(unidrnd(length(uindplus))); end;
            end;
            % transform into the actual action
            U(:, k+1, trial) = Uflat(:, uindplus);
            
            % compute temporal difference and update Q-function
            tempdiff = R(1, k+1, trial) + cfg.gamma * Q(xindplus, uindplus) - Q(xind, uind);
            if cfg.lambda > 0,  % eligibility trace based update
                E(xind, uind) = 1;                  % replace trace of crt state-action pair
                Q = Q + (alpha * tempdiff) .* E;    % update Q-function
            ...
            t = cputime; 
            k = k + 1; xind = xindplus; uind = uindplus;
        end;
