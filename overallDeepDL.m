...
% % % %    wole_wave(:,num_rep*(i-1)+1:num_rep*(i-1)+num_rep)=repmat(eta_irreg_total(:,p),1,num_rep);  
% % % %
% % % %     end
...
% initial network
prob=0.6;  %%%% probability of staying in the same sea state
...
% for episode=1:size(ini_epis_state_idx,2)
    ...
    init_st=datasample(1:size(states_space,1),1);   %% initial state
...
%         end
        ...
        [All_params3,next_state_idx, PptoAvg]=SIMULATOR2(FeWithTime,A1,B1,K1,B2,K2,tsim,StepT_SIM,state_idx,init_st);  ...
            % initiaL STATE
        ...
        disp(['current state : ' num2str(state_idx) ' next state : ' num2str(next_state_idx)...
            ' taken action : ' num2str(action(action_idx,:))]);
        ...
        if ove_count> NN_dd  %% start traing network
            ...
            Memory_Stor(ove_count,:)=[state_idx,action_idx,next_reward,next_state_idx,states_space(state_idx,:),...
                target(state_idx,:)];
        ...
        %dnn=trainDBN(dnn,states_space(state_idx,:),target(state_idx,:));
            
            
        else
            
            Q(state_idx,action_idx)=Q(state_idx,action_idx)+alpha*(next_reward+gamma*max(Q(next_state_idx,:))-...
