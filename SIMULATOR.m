...
function [All_params3, next_state_indx, PptoAvg]=SIMULATOR(FeWithTime,A1,B1,K1,B2,K2,tsim,StepT_SIM,state_idx,prob,init_st)
% every 5 minutes
...
PptoAvg=mean(Ppto(5/StepT_SIM:end-5/StepT_SIM));

%% Frequency, magnitude and phase of Fe, Fr, Vel, Pos signals

Fs=500;
Fe=Fe(5/StepT_SIM:end-5/StepT_SIM);  %%% moise extraction
Fe = Fe - mean(Fe);
...
%% Fr parameters
Fr=Fr(5/StepT_SIM:end-5/StepT_SIM);  %%% moise extraction
Fr = Fr - mean(Fr);
%%% amplitude and phase
...
%% position parameters
Pos=Pos(5/StepT_SIM:end-5/StepT_SIM);  %%% moise extraction
Pos = Pos - mean(Pos);
%%% amplitude and phase
...
%% Velocity parameters
Vel= Vel(5/StepT_SIM:end-5/StepT_SIM);  %%% moise extraction
Vel = Vel - mean(Vel);
...
Fe_Params(1,2)=Fe_Params(1,2);
Fe_Params(1,1)=Fe_Params(1,1);
...
% % % %
% % % %   p=find(state_space(:,1)==state_space(indxF,1))
% % % %   d=find(state_space(:,2)==state_space(indxM,2))
% % % %   f=find(state_space(:,3)==round(B2_norm) )
% % % %   j=find(state_space(:,4)==round(K2_norm))

r=rand; % get 1 uniform random number
x=sum(r>=cumsum([0, 1-prob, prob])); % check it to be in which probability area
% choose either explore or exploit

% if x == 1  %with probability of Prob the same state otherwise random
%     
%     st_chosen=init_st;
%     
% else
%     
%     oth_SeaState=datasample(1:size(states_space,1),1); % choose 1 action randomly (uniform random distribution)
%     st_chosen=oth_SeaState;
% end
