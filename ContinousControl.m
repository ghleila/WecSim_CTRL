...
NN_dd=2; %% this is for the case we want to run the simulator and observe reward without using NN, somhow pre-NN data
...
for ove_count=1:1500
    ...
    if ove_count> NN_dd
        ...
        B2_prime= net_Actor1(SS_n(next_state_idx,:)');
        K2_prime= net_Actor2(SS_n(next_state_idx,:)');

        action_idx = find(SS_n(:,3)==B2_prime & SS_n(:,4)==K2_prime); % id of the chosen action
...
% % % %     K2=action(action_idx,2);  %% stiffness
    ...
    if ove_count>= NN_dd  %% start traing network
        ...
        Actor1_Out_B2=SS_n(Actor_idx,3);
        Actor2_Out_K2=SS_n(Actor_idx,4);
        
        net_Actor1 = feedforwardnet([4]);
        [net_Actor1,tr] = train(net_Actor1,Input',Actor1_Out_B2');
        
        net_Actor2 = feedforwardnet([4]);
        [net_Actor2,tr] = train(net_Actor2,Input',Actor2_Out_K2');
        
        B_prime= net_Actor1(SS_n(next_state_idx,:)');
        K_prime= net_Actor2(SS_n(next_state_idx,:)');
